...
/***************************************************************/
export function generarTablaFromStringJSON(string, botones = false, onEditCallback, onDeleteCallback) {
  if (!string) {
    return '';
  }
  ...
  if (isArray) {
    ...
    const rows = data.map((producto, index) => {
      const cells = headers.map(header => {
        if (header === 'imagen') {
          return `<td><img src="${producto[header]}" alt="${producto['nombre']}" style="width: 50px; height: 50px;" /></td>`;
