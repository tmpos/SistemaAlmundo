...
/************************************************************************/
const datoscamposBranch_office = ref({})
/************************************************************************/
...
const Branch_officeEditar = ref(null);
/************************************************************************/
async function limpiarCamposCrear() {
datoscamposBranch_office.value = {}
await campos();
}
/************************************************************************/
watchEffect(() => {
...
const selectOption = async(row: any, option: string) => {
  ...
  const datos = row;
  if(option === 'Editar'){
arrayIMG.value = await peticiones(link.value+api.value+'/peticionimagenes',{"origen":`${'../vista/img/branch_office/'+datos.imagen}`},'POST',tokenCifrado.value)
additionalData.value = {ruta: '../vista/img/branch_office/'+datos.imagen};
visible.value = true;
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (contrasenaIngresada === token.value || contrasenaIngresada === tokenCorto.value) {
                        const datosFactura = await peticionesFetch(`${link.value}${api.value}`, `borrarporcampo/branch_office`, { campo: 'id', valor: datos.id }, tokenCifrado.value, 'POST');
...
async function campos() {
  const campos = await arrayToObjetoFromTabla(link.value+api.value, tokenCifrado.value, 'branch_office', true,camposArray,'usuario');
  datoscamposBranch_office.value = campos;
}
/************************************************************************/
onMounted(async () => {
...
await crearTablaSiNoExiste(link.value, api.value, 'branch_office', camposArray, tokenCifrado.value,toast);
        ...
        if (result.isConfirmed) {
            ...
            if (password) {
               if (password === token.value || password === tokenCorto.value) {
                    const envioDatos = await borrarTodoslosDatos(link.value + api.value + '/borrartodo', 'branch_office', tokenCifrado.value);
                    if (envioDatos[0] == 'ok') {
                        fetchAndSetupData();
...
async function funcionActualizar() {
  const url = link.value+api.value+"/actualizarcampos/branch_office";
...
async function funcionCrear() {
  const url = link.value+api.value+"/insertar/branch_office";
  if (datoscamposBranch_office.value.hasOwnProperty('created_at')) {
    datoscamposBranch_office.value.created_at = nfecha('timestamp');
    datoscamposBranch_office.value.updated_at = nfecha('timestamp');
  }
  const envioDatos = await enviarDatosPorPost(url, datoscamposBranch_office.value, tokenCifrado.value);
  if (envioDatos[0] == 'ok') {
    await fetchAndSetupData();
            ...
            if (password) {
                if (password === token.value || password === tokenCorto.value) {
                    ...
                    if (ids.length > 0) {
                        for (const id of ids) {
                            try {
                                const envioDatos = await eliminarDatos(`${link.value}${api.value}/borrar/branch_office`, id, tokenCifrado.value);
                            } catch (error) {
...
/************************************************************************/
const filteredBranch_office = computed(() => {
...
const getImageSrc = (imagen) => {
  return `${link.value}/vista/img/branch_office/${datoscampos.value.imagen}/${imagen}`;
};
...
const generatePDF = () => {
  ...
  doc.save('tabla_branch_office.pdf');
};
/************************************************************************/
</script>
<template>
        ...
        <div class="fixed inset-0 overflow-y-auto">
         <div class="flex min-h-full items-start justify-center px-4 py-8">
           <DialogPanel class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark animate__animated animate__slideInDown">
             ...
             <div class="text-lg font-bold bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">Modal Agregar Branch_office</div>
             <div class="p-5">
  <fieldset class="border p-3 round mb-2">
    <legend class="float-none w-auto px-2">Branch_office</legend>
     <form id="formularioActualizarBranch_office" action="" method="">
...
<input type="input" v-model="datoscamposBranch_office.name" name="name"  class="form-input " id="nameAgregarDatos"  v-mayuscula placeholder="name" maxlength="250">
</div>
<div class="form-group " hidden>
<label for="created_atAgregarDatos">{{t('CREATED_AT')}}</label>
<input type="input" v-model="datoscamposBranch_office.created_at" name="created_at"  class="form-input " id="created_atAgregarDatos"   placeholder="created_at" maxlength="">
</div>
<div class="form-group " hidden>
<label for="updated_atAgregarDatos">{{t('UPDATED_AT')}}</label>
<input type="input" v-model="datoscamposBranch_office.updated_at" name="updated_at"  class="form-input " id="updated_atAgregarDatos"   placeholder="updated_at" maxlength="">
</div>
<div class="form-group " hidden>
<label for="usuarioAgregarDatos">{{t('USUARIO')}}</label>
<input type="input" v-model="datoscamposBranch_office.usuario" name="usuario"  class="form-input " id="usuarioAgregarDatos"   placeholder="usuario" maxlength="250">
</div>
